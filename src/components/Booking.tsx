...
export default function Booking() {
  ...
  return (
    <section id="booking" className="relative overflow-hidden py-24 sm:py-32">
      <Blob className="-right-40 bottom-0 h-[24rem] w-[24rem]" color="var(--color-sage)" opacity={0.28} />

      <div className="relative mx-auto grid max-w-6xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
        <Reveal>
          <h2 className="display text-4xl text-espresso sm:text-5xl">{tt(c.booking.title)}</h2>
          <p className="mt-6 max-w-md text-[1.02rem] leading-relaxed text-espresso-soft">
            {tt(c.booking.intro)}
          </p>
        ...
        <Reveal delay={80}>
          <form
            onSubmit={submit}
            className="rounded-[2rem] border border-espresso/10 bg-white/55 p-6 backdrop-blur sm:p-9"
          >
            ...
            <button
              type="submit"
              className="mt-7 flex w-full items-center justify-center gap-2.5 rounded-full bg-espresso px-7 py-4 text-xs tracking-[0.2em] uppercase text-cream transition-colors hover:bg-espresso-soft"
            >
